...
import { northernProvinces, allCities } from "@/content/coverage";
...
export const metadata: Metadata = seoMetadata({
  title: "مناطق تحت پوشش امداد خودرو؛ تهران، کرج و شمال",
  description: "پوشش امداد خودرو تهران، کرج، گیلان، مازندران و گلستان؛ راهنمای شهرها، محله‌ها و مسیرها با بررسی موقعیت، تجهیزات و ظرفیت اعزام.",
  path: "/coverage",
  keywords: ["مناطق تحت پوشش امداد خودرو", "امداد خودرو تهران", "امداد خودرو کرج", "پوشش امداد خودرو"],
...
export default function CoveragePage() {
  return <SubpageShell>
    <StructuredData data={breadcrumbSchema([{ name: "صفحه اصلی", path: "/" }, { name: "مناطق تحت پوشش" }], "/coverage")} /><StructuredData data={webPageSchema({ name: "مناطق تحت پوشش امداد خودرو خودرو چاره", description: "مناطق تحت پوشش امداد خودرو در تهران، کرج، گیلان، مازندران و گلستان.", path: "/coverage", breadcrumb: true })} />
    <section className="relative overflow-hidden bg-ink text-white"><div className="site-container grid items-center gap-8 py-12 lg:grid-cols-[.9fr_1.1fr]" dir="ltr"><div className="relative min-h-72 overflow-hidden rounded-[1.8rem] border border-white/15"><Image src="/images/support-technician-night.webp" alt="معرفی خدمات شبانه‌روزی خودرو چاره" fill priority quality={76} sizes="(min-width:1024px) 45vw,100vw" className="object-cover object-[28%_center]" /><div className="absolute inset-0 bg-gradient-to-t from-ink/55 via-transparent to-transparent" /></div><div dir="rtl"><p className="text-xs font-black text-orange-300">پاسخ‌گویی ۲۴ ساعته، ۷ روز هفته</p><h1 className="mt-4 text-3xl font-black leading-[1.55] md:text-5xl">مناطق تحت پوشش امداد خودرو؛ تهران، کرج و شمال</h1><p className="mt-5 max-w-2xl text-sm leading-8 text-slate-300">خودرو چاره درخواست خدمات خودرو را در تهران، کرج و سه استان گیلان، مازندران و گلستان هماهنگ می‌کند. علاوه بر امداد در محل، یدک‌کش و خودروبر، امکان درخواست مکانیک سیار، باتری، پنچرگیری، سوخت‌رسانی و کارواش سیار نیز فراهم است. زمان اعزام امداد به ترافیک، موقعیت و نوع خدمت بستگی دارد.</p><nav className="mt-6 flex flex-wrap gap-2" aria-label="خدمات قابل درخواست در مناطق تحت پوشش"><Link href="/services/roadside-assistance" className="rounded-full border border-white/20 bg-white/5 px-3 py-2 text-xs font-bold transition hover:border-orange-300">امداد در محل</Link><Link href="/services/tow-truck" className="rounded-full border border-white/20 bg-white/5 px-3 py-2 text-xs font-bold transition hover:border-orange-300">یدک‌کش و خودروبر</Link><Link href="/services/mobile-mechanic" className="rounded-full border border-white/20 bg-white/5 px-3 py-2 text-xs font-bold transition hover:border-orange-300">مکانیک سیار</Link><Link href="/services/mobile-carwash" className="rounded-full border border-white/20 bg-white/5 px-3 py-2 text-xs font-bold transition hover:border-orange-300">کارواش سیار</Link></nav></div></div></section>
    <Section title="تهران، کرج و شبکه فعال شمال کشور"><p>پوشش عملیاتی سه استان شمالی بر اساس اعلام مدیریت مجموعه فعال است. اعزام به شهرها، روستاها و راه‌های فرعی با دریافت موقعیت و بررسی ظرفیت و ایمنی انجام می‌شود. داشتن صفحه محله به معنی شعبه ثابت، موجودی تضمینی قطعه یا زمان رسیدن یکسان نیست.</p><LinkGrid links={[{ title: "امداد خودرو تهران", href: "/تهران" }, { title: "امداد خودرو کرج", href: "/کرج" }, { title: "امداد خودرو شمال", href: "/شمال" }, ...northernProvinces.map((province) => ({ title: `امداد خودرو ${province.name}`, href: `/${province.slug}` })), { title: "راهنمای جاده‌های شمال", href: "/roads" }]} /></Section>
    {northernProvinces.map((province) => <Section key={province.slug} title={`شهرهای ${province.name}`}><p>{province.access}</p><LinkGrid links={allCities.filter((city) => city.province === province.slug).map((city) => ({ title: `امداد خودرو ${city.name}`, href: `/${city.slug}` }))} /></Section>)}
    <Section title="راهنمای محله‌های تهران و محدوده‌های کرج"><p>برای نکات دسترسی، پارکینگ و موقعیت‌های مهم هر محدوده، صفحه مرتبط را بخوانید. فهرست پایین‌تر، نام‌های مرجع برای اعلام موقعیت است و مرزبندی رسمی محله یا تضمین دسترسی به همه نقاط محسوب نمی‌شود.</p><LinkGrid links={neighborhoods.map((area) => ({ title: `امداد خودرو ${area.name}`, href: `/${area.citySlug}/${area.slug}` }))} /></Section>
    <section className="site-container mt-8" dir="rtl"><div className="overflow-hidden rounded-2xl border border-slate-200 bg-[#071a2e] shadow-card"><div className="grid items-center lg:grid-cols-[1.35fr_.65fr]" dir="ltr"><div className="relative min-h-[340px] sm:min-h-[460px]"><Image src="/images/coverage-iran-glass-v2.webp" alt="تصویر شماتیک محدوده پوشش خودرو چاره" fill loading="lazy" quality={75} sizes="(min-width:1024px) 68vw,100vw" className="object-cover" /></div><div className="p-6 text-white md:p-8" dir="rtl"><p className="text-xs font-black text-emerald-300">نمای شماتیک؛ نه نقشه زنده</p><h2 className="mt-3 text-2xl font-black">تهران، کرج و سه استان شمالی</h2><p className="mt-4 text-sm leading-8 text-slate-300">این تصویر، شماتیک و غیرزنده است. شبکه تهران، کرج، گیلان، مازندران و گلستان فعال است؛ فهرست متنی شهرها و تأیید پذیرش برای موقعیت دقیق، مبنای بررسی پوشش هستند.</p><div className="mt-5 flex flex-wrap gap-2"><span className="rounded-full border border-emerald-300/30 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-200">تهران · فعال</span><span className="rounded-full border border-emerald-300/30 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-200">کرج · فعال</span><Link href="/شمال" className="rounded-full border border-emerald-300/30 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-200">گیلان، مازندران و گلستان · فعال</Link></div></div></div></div></section>
